...
export function SubmissionsByMonthOfYearCard({ courseNm, chartData, colors }: SubmissionsByMonthOfYearCardProps) {
    return (
        <ResizableCard defaultHeight={340}>
            ...
            <CardContent className="px-2 py-0">
                <StackedOkKoBarChart
                    data={chartData.submissionsByMonth}
                    colors={colors}
                    exportFileName={`${courseNm}-submissions-by-month`}
                />
            </CardContent>
        </ResizableCard>
    )
