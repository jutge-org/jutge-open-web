import { decodeBase64Bytes, decodeBase64Utf8 } from '@/lib/base64'
import {
...
function decodeTestcaseAnalysis(
    ...
    const decoded: SubmissionTestcaseAnalysisData = {
        ...
        input: decodeBase64Utf8(analysis.input_b64),
        output: '',
        expected: '',
    }
    ...
    decoded.output = decodeBase64Utf8(analysis.output_b64)
    decoded.expected = decodeBase64Utf8(analysis.expected_b64)
    return decoded
}
...
export async function fetchSubmissionCode(
    ...
    const resolvedProblemId = await resolveProblemId(client, key, preferredLanguageId)
    ...
    return {
        body: decodeBase64Bytes(codeB64),
        contentType:
...
export async function fetchSubmissionDetail(
    ...
    key: string,
    submission_id: string,
    options?: { isAdministrator?: boolean; isExamOrContest?: boolean; preferredLanguageId?: string | null },
): Promise<SubmissionDetailData | null> {
    const preferredLanguageId = options?.preferredLanguageId ?? null
    const resolvedProblemId = await resolveProblemId(client, key, preferredLanguageId)
    if (!resolvedProblemId) {
        return null
    }
...
export async function fetchSubmissionTestcaseAnalysis(
    ...
    try {
        ...
        const [analysis, tables, abstractProblem] = await Promise.all([
            ...
            client.tables.get(),
            fetchAbstractProblem(client, problem_nm),
        ])
